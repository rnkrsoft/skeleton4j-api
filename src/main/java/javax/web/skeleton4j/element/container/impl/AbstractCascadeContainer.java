...
import javax.web.skeleton4j.element.container.WebCascadeContainer;
...
@Slf4j
public abstract class  AbstractCascadeContainer extends AbstractContentContainer implements WebCascadeContainer {

    public AbstractCascadeContainer(WebComponentRegistry componentRegistry, WebPage webPage, WebThemePack themePack, ElementInfo column, String parentId, String name) {
        super(componentRegistry, webPage, themePack, column, parentId, name);
    }

    protected AbstractCascadeContainer(AbstractCascadeContainer src) {
        super(src);
    }
    ...
    public void getCodes(WebContext ctx) {
        if (isCompilable()) {
            File bufferFile = new File(getCachedFilePath(WebCascadeContainer.class.getSimpleName(), ctx.getTheme()));
    ...
    public void scripts(String theme, Collection<WebScript> scripts1) {
        scripts1.addAll(WebComponentRegistryFactory.getInstance().getThemePack(theme).getScripts(WebCascadeContainer.class));
    }

    @Override
    public void styles(String theme, Collection<WebStyle> styles1) {
        styles1.addAll(WebComponentRegistryFactory.getInstance().getThemePack(theme).getStyles(WebCascadeContainer.class));
    }

    @Override
    public Class<? extends WebElement> getElementClass() {
        return WebCascadeContainer.class;
    }
}
