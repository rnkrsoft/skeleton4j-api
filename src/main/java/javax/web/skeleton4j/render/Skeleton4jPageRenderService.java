...
public interface Skeleton4jPageRenderService {
     ...
     * @return 渲染结果
     */
    WebRenderedResult render(EnumStringCode enumStringCode);
    /**
     * 将错误信息渲染成页面
     * @param json JSON
     * @return 渲染结果
     */
    WebRenderedResult render(String json);
    /**
     * 将错误信息渲染成页面
     * @param rspCodeReadable 错误上下文
     * @return 渲染结果
     */
    WebRenderedResult render(RspCodeReadable rspCodeReadable);
    /**
     * 将错误信息渲染成页面
     * @param context 错误上下文
     * @return 渲染结果
     */
    WebRenderedResult render(ErrorContext context);
    /**
     * 渲染页面，将预编译好的页面对象，进行最终的渲染（主要是根据输入的用户进行菜单的渲染）,菜单对应的页面只要有一个接口有权限，都暂时该界面出来
     * @param context 渲染上下文
     * @return 渲染结果
     */
    WebRenderedResult render(RenderContext context);
}
